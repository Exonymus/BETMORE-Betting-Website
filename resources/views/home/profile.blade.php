...
@section('content')
    @if(Auth::user()->role->name == 'user' and Auth::user()->id != $user->id)
                        ...
                        <div class="e-profile">
                            ...
                            <div class="tab-content pt-3" id="profile-tabs_content">
                                <div class="tab-pane fade show active"
                                    ...
                                    <form action="{{ route('home.profile.update', ['id' => $user->id]) }}" method="post" enctype="multipart/form-data" class="form row justify-content-around">
                                        @csrf
    ...
    <script>
        ...
        $("#take-credit-btn").click(function () {
            ...
            if (amountToTake > 0 && amountToTake <= {{(int)($user->exp / 5000) * 1000}}) {
                ...
                $.ajax({
                    url: '{{ route("home.profile.debt", ['id' => $user->id]) }}',
                    method: 'POST',
                    data: debtData,
                    contentType: false,
                    processData: false,
        ...
        $("#pay-debt-btn").click(function () {
            ...
            if (amountToTake > 0) {
                ...
                // Process taking credit (you may need to implement this logic)
                $.ajax({
                    url: '{{ route("home.profile.debt.pay", ['id' => $user->id]) }}',
                    method: 'POST',
                    data: debtData,
                    contentType: false,
                    processData: false,
        ...
        document.getElementById('upload-avatar-inp').addEventListener('change', function () {
            ...
            if (fileInput.files.length > 0) {
                ...
                $.ajax({
                    url: '{{ route("home.profile.update-avatar", ['id' => $user->id]) }}',
                    method: 'POST',
                    data: formData,
                    contentType: false,
                    processData: false,
